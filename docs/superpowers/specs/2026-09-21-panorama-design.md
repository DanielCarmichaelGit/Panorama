...
- **Events**: `ticket.created`, `ticket.updated` (carries `changed[]`, the names of the fields that changed, `tagIds` included when tags were added or removed), `ticket.moved` (from, to), `ticket.flag_set`, `ticket.flag_cleared`, `ticket.linked`, `ticket.unlinked` (from, to, kind), `evidence.added` (type, result), `comment.added`, `epic.created`, `epic.updated`, `tag.created`, `tag.archived`, `field.created`, `field.updated`, `field.archived` (milestone 2b), `tag.updated` (changed[]), `lane.created`, `lane.updated` (changed[]), `lane.reordered` (ids), `lane.deleted`, `evidence_type.created`, `evidence_type.deleted` (milestone 2c; evidence types are global, so those two carry no projectId), `timer.started`, `timer.stopped`, `cost.added`, `trigger.fired`, `ticket.due_passed`.
