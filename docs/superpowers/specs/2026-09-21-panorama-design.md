...
2b. **Ticket model and creation** (added by the owner on 2026-09-24): epics and dependencies pulled forward from milestone 4, tags, success criteria, per-project custom fields (kinds text, number, date, select, checkbox, and file, whose value is `{attachmentId}` naming one of the ticket's own attachments; the server refuses any other ticket's), a Settings view (fields, tags, epics, lanes and requirements, evidence types), a full-screen create dialog, and a shared Picker replacing every native select.
